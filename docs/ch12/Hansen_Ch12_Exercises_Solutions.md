...
**数值验证：** `Hansen_Ch12_Exercises_Solutions.ipynb`（12.23 AJR、12.25 Card、12.27 AK + 理论结论的蒙特卡洛验证）
...
> **要点：** IV 是**一致但有偏**的（小样本偏，大样本一致）。这正是 bootstrap 在 IV 上不稳的原因（12.24）。
...
## Exercise 12.21　恰好识别：$\hat\beta_1=\hat\lambda_1$

**解答：** 恰好识别时，Frisch–Waugh：对 $Z_1$ 残差化后，结构与约简式对 $Z_1$ 的系数一致 ⇒ $\hat\beta_1=\hat\lambda_1$（OLS $Y$ 对 $X_1,Z_2$ 中 $X_1$ 的系数）。

---

## Exercise 12.22　GLS=IV（同 12.2）

$Z_i=X_i/\sigma^2(X_i)$ 使 GLS=IV。

---

## Exercise 12.23（AJR2001）　制度与GDP：经典 IV 实证
...
## Exercise 12.24　IV 的 bootstrap SE 不稳
...
## Exercise 12.25（Card1995）　教育回报：就近上大学作工具
...
## Exercise 12.26　Card IV 的 BC 区间

IV 基线：BC percentile 区间适合有偏估计量。IV bootstrap SE 往往**不宜单独强调**（见 12.24）；应报 BC 区间。

---

## Exercise 12.27（AK1991）　黑人子样本（3 QOB）
...
| 习题 | notebook 内容 |
|------|----------------|
| 12.23 | AJR2001 OLS/RF/2SLS/ILS/控制函数 + 一阶段 F + Sargan code cell |
| 12.25 | Card1995 2SLS + 一阶段 F code cell |
| 12.27 | AK1991 黑人子样本 3-QOB 2SLS + 一阶段 F code cell |
