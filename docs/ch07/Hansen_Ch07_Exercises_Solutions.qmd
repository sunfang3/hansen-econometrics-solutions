...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch07.R` 优先；否则为带注释的翻译草稿）。
...
# PY: vals = np.array([[1,1],[1,-1],[-1,1],[-1,-1]], float)
# PY: p = np.array([3/8,1/8,1/8,3/8])
# PY: X1, X2 = vals[:,0], vals[:,1]
# PY: same = (X1==X2)
# PY: sig2 = np.where(same, 5/4, 1/4)

# PY: print('E[X1]=', np.sum(p*X1))
# PY: print('E[X1^2]=', np.sum(p*X1**2))
# PY: print('E[X1X2]=', np.sum(p*X1*X2))
# PY: print('E[e^2]=', np.sum(p*sig2))
# PY: print('E[X1^2 e^2]=', np.sum(p*(X1**2)*sig2))
# PY: print('E[X1X2 e^2]=', np.sum(p*X1*X2*sig2))
# PY: print('targets: 0,1,0.5,1,1,0.875')
...
# PY: CPS = Path("../../hansen/econometrics/data/cps09mar/cps09mar.xlsx")
# PY: if not CPS.exists():
...
# PY: s = df[(df.race==1)&(df.female==0)&(df.hisp==1)].copy()
# PY: y = s.lwage.to_numpy(float)
# PY: X = np.c_[s.education, s.experience, s.exp2, np.ones(len(s))]
...
# PY: u = X * (e/(np.clip(1-h,1e-12,None)))[:,None]  # HC3
# PY: V = XXinv @ (u.T@u) @ XXinv
# PY: se = np.sqrt(np.diag(V))
# PY: print(pd.DataFrame({"beta":beta,"HC3_SE":se}, index=names))
# PY: print("n=", n)
...
# PY: den = b2 + 0.2*b3
# PY: theta = b1/den
# PY: grad = np.array([1/den, -b1/den**2, -0.2*b1/den**2, 0.0])
# PY: se_theta = float(np.sqrt(grad @ V @ grad))
# PY: z90 = stats.norm.ppf(0.95)
# PY: print(f"theta = {theta:.4f}")
# PY: print(f"se(theta) = {se_theta:.4f}")
# PY: print(f"90% CI = [{theta - z90*se_theta:.4f}, {theta + z90*se_theta:.4f}]")
...
# PY: x = np.array([12.0, 20.0, 4.0, 1.0])  # exp2/100 = 400/100=4
# PY: m = float(x @ beta)
# PY: se_m = float(np.sqrt(x @ V @ x))
# PY: z95 = 1.96
# PY: print(f"m(12,20) = {m:.4f}")
# PY: print(f"95% CI = [{m-z95*se_m:.4f}, {m+z95*se_m:.4f}]")
...
# PY: s2 = float(np.sum(e**2)/(n-k))
# PY: x = np.array([16.0, 5.0, 0.25, 1.0])
# PY: yhat = float(x @ beta)
# PY: se_f = float(np.sqrt(x @ V @ x + s2))
# PY: z80 = stats.norm.ppf(0.90)
# PY: lo, hi = yhat - z80*se_f, yhat + z80*se_f
# PY: print(f"point forecast log wage = {yhat:.4f}")
# PY: print(f"80% PI log wage = [{lo:.4f}, {hi:.4f}]")
# PY: print(f"80% PI wage = [{np.exp(lo):.2f}, {np.exp(hi):.2f}]")
...
# import numpy as np
# PY: rng = np.random.default_rng(7)

# Ex 7.1: 短回归 plim ≠ β（大样本 OVB）
# PY: n = 200000
# PY: X1 = rng.standard_normal(n)
# PY: X2 = 0.5 * X1 + rng.standard_normal(n)
# PY: Y = 1.0 * X1 + 2.0 * X2 + rng.standard_normal(n)
# PY: b1s = np.sum(X1 * Y) / np.sum(X1**2)
# PY: plim = 1.0 + np.mean(X1 * X2) / np.mean(X1**2) * 2.0
# PY: print(f"[7.1] 短回归 β̂₁={b1s:.3f}, 公式 plim={plim:.3f}  (≠ 真 β₁=1, OVB)")

# Ex 7.24: 自变量乘性测量误差 → 衰减偏差
# PY: Xstar = np.abs(rng.standard_normal(n)) + 0.5
# PY: v = rng.uniform(0.5, 1.5, n)
# PY: Y = 2.0 * Xstar + rng.standard_normal(n)
# PY: X = Xstar * v
# PY: bhat = np.sum(X * Y) / np.sum(X**2)
# PY: plim = 2.0 * np.mean(v) / np.mean(v**2)
# PY: print(f"[7.24] β̂={bhat:.3f}, plim=β·E[v]/E[v²]={plim:.3f}  (< β=2, 衰减)")

# Ex 7.15: 非标准估计量 ΣX³Y/ΣX⁴ 的渐近方差 = E[X⁶e²]/(E[X⁴])²
# PY: N, reps = 80, 4000
# PY: mhats = []
# PY: Xall = rng.standard_normal((reps, N)); eall = rng.standard_normal((reps, N))
# PY: for r in range(reps):
# PY:     Xs = Xall[r]; Ys = Xs + eall[r]
# PY:     mhats.append(np.sum(Xs**3 * Ys) / np.sum(Xs**4))
# PY: X = rng.standard_normal(400000); e = rng.standard_normal(400000)
# PY: avar = np.mean(X**6 * e**2) / (np.mean(X**4))**2
# PY: print(f"[7.15] var(β̂)·N={np.var(mhats)*N:.3f} ≈ 公式 avar={avar:.3f}")

# delta method: θ=β₁β₂ 的 95% CI 覆盖应≈0.95
# PY: n, reps = 200, 20000
# PY: b1, b2, theta = 1.0, 0.5, 0.5
# PY: cov = 0
# PY: for r in range(reps):
# PY:     X = np.c_[rng.standard_normal(n), rng.standard_normal(n), np.ones(n)]
# PY:     e = rng.standard_normal(n); Y = X @ [b1, b2, 0.0] + e
# PY:     bh = np.linalg.solve(X.T @ X, X.T @ Y)
# PY:     V = np.linalg.inv(X.T @ X)
# PY:     G = np.array([b2, b1, 0.0])
# PY:     se = np.sqrt(G @ V @ G)
# PY:     lo, hi = bh[0]*bh[1] - 1.96*se, bh[0]*bh[1] + 1.96*se
# PY:     cov += (lo <= theta <= hi)
# PY: print(f"[delta] θ=β₁β₂ 的 95% CI 覆盖={cov/reps:.4f}  (应≈0.95)")
