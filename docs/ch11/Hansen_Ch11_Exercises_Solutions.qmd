---
title: "Bruce Hansen《Econometrics》第 11 章习题解答"
subtitle: "Chapter 11 Multivariate Regression"
...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch11.R` 优先；否则为带注释的翻译草稿）。
...
# PY: X = rng.normal(size=(n, k))
# PY: X = np.column_stack([X, np.ones(n)])  # add intercept? use k=2 without intercept for simplicity
# PY: X = rng.normal(size=(n, 2))
# PY: beta1 = np.array([1.0, -0.5])
# PY: beta2 = np.array([0.2, 0.8])
# correlated errors
# PY: z = rng.normal(size=(n, 2))
# cov [[1, 0.6],[0.6, 1]]
# PY: L = np.array([[1.0, 0.0], [0.6, np.sqrt(1-0.36)]])
# PY: e = z @ L.T
# PY: Y1 = X @ beta1 + e[:, 0]
# PY: Y2 = X @ beta2 + e[:, 1]

# PY: Q = X.T @ X / n
# PY: b1 = np.linalg.solve(X.T @ X, X.T @ Y1)
# PY: b2 = np.linalg.solve(X.T @ X, X.T @ Y2)

# theoretical avar blocks (homosk): Sigma_jl * Qinv / n
# PY: Sig = np.cov(e.T, bias=True)
# PY: Qinv = np.linalg.inv(Q)
# Monte Carlo many reps for sampling var of sqrt(n)(b-beta)
# PY: B = 400
# PY: rng2 = np.random.default_rng(1)
# PY: diffs = []
# PY: for _ in range(B):
# PY:     X = rng2.normal(size=(n, 2))
# PY:     z = rng2.normal(size=(n, 2))
# PY:     e = z @ L.T
# PY:     Y1 = X @ beta1 + e[:, 0]
# PY:     Y2 = X @ beta2 + e[:, 1]
# PY:     b1 = np.linalg.solve(X.T @ X, X.T @ Y1)
# PY:     b2 = np.linalg.solve(X.T @ X, X.T @ Y2)
# PY:     diffs.append(np.concatenate([np.sqrt(n)*(b1-beta1), np.sqrt(n)*(b2-beta2)]))
# PY: S = np.cov(np.array(diffs).T)
# theory: kron(Sigma, Q^{-1}) but Q random; use E[Q]=I
# PY: V_theory = np.kron(np.array([[1, 0.6], [0.6, 1]]), np.eye(2))
# PY: print("MC cov of sqrt(n)(beta_hat - beta) [approx]:\n", np.round(S, 3))
# PY: print("Theory Sigma ⊗ I_2:\n", V_theory)
# PY: print("Frobenius relative error:", np.linalg.norm(S - V_theory) / np.linalg.norm(V_theory))
...
# import numpy as np
# PY: rng = np.random.default_rng(11)

# PY: n, reps = 4000, 3000
# PY: rho = 0.7
# PY: L = np.array([[1.0, 0.0], [rho, np.sqrt(1 - rho**2)]])          # 误差协方差 [[1,ρ],[ρ,1]] 的 Cholesky
# PY: b1, b2 = 1.0, 0.5
...
# PY:     X2 = rng.standard_normal(n) + 0.3 * X1                      # 不同回归元(但相关)
# PY:     z = rng.standard_normal((n, 2)); e = z @ L.T
# PY:     Y1 = b1 * X1 + e[:, 0]; Y2 = b2 * X2 + e[:, 1]
# PY:     ols1.append(np.sum(X1*Y1) / np.sum(X1**2))
# PY:     Sinv = np.linalg.inv(np.cov(e.T))                           # 已知 Σ 的 GLS = SUR
# PY:     A = np.array([[np.sum(X1**2)*Sinv[0,0], np.sum(X1*X2)*Sinv[0,1]],
# PY:                   [np.sum(X1*X2)*Sinv[1,0], np.sum(X2**2)*Sinv[1,1]]])
# PY:     rhs = np.array([Sinv[0,0]*np.sum(X1*Y1) + Sinv[0,1]*np.sum(X1*Y2),
# PY:                     Sinv[1,0]*np.sum(X2*Y1) + Sinv[1,1]*np.sum(X2*Y2)])
# PY:     sur1.append(np.linalg.solve(A, rhs)[0])
# PY: print(f"[11.12] 回归元不同+误差相关: var(OLS)={np.var(ols1):.5f} > var(SUR)={np.var(sur1):.5f}  (SUR 更有效)")
...
# PY:     z = rng.standard_normal((n, 2)); e = z @ L.T
# PY:     Y1 = b1 * X + e[:, 0]; Y2 = b2 * X + e[:, 1]
# PY:     ols1c.append(np.sum(X*Y1) / np.sum(X**2))
# PY:     Sinv = np.linalg.inv(np.cov(e.T))
# PY:     A = np.array([[np.sum(X**2)*Sinv[0,0], np.sum(X**2)*Sinv[0,1]],
# PY:                   [np.sum(X**2)*Sinv[1,0], np.sum(X**2)*Sinv[1,1]]])
# PY:     rhs = np.array([Sinv[0,0]*np.sum(X*Y1) + Sinv[0,1]*np.sum(X*Y2),
# PY:                     Sinv[1,0]*np.sum(X*Y1) + Sinv[1,1]*np.sum(X*Y2)])
# PY:     sur1c.append(np.linalg.solve(A, rhs)[0])
# PY: print(f"[11.5/11.6] 共同回归元: var(OLS)={np.var(ols1c):.6f} = var(SUR)={np.var(sur1c):.6f}, "
# PY:       f"逐样本|OLS-SUR|均值={np.mean(np.abs(np.array(ols1c) - np.array(sur1c))):.1e}  (SUR = OLS)")
