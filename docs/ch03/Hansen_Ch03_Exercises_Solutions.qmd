---
title: "Bruce Hansen《Econometrics》第 3 章习题解答"
subtitle: "Chapter 3 The Algebra of Least Squares"
...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch03.R` 优先；否则为带注释的翻译草稿）。
...
# PY: candidates = [
# PY:     Path("../../hansen/econometrics/data/cps09mar/cps09mar.xlsx"),
# PY:     Path("/home/fang/Project/zhihu-paper/p1/hansen/econometrics/data/cps09mar/cps09mar.xlsx"),
# PY: ]
# PY: DATA = next(p for p in candidates if p.exists())
# PY: print("data:", DATA.resolve())

# PY: df = pd.read_excel(DATA)
...
# PY:     & (df["experience"] < 45)
# PY: )
# PY: sub = df.loc[mask_349].copy()
# PY: print("n =", len(sub))
# PY: sub[["education", "experience", "lwage"]].describe()
...
# PY: def ols(y, X, names=None):
# PY:     beta = np.linalg.lstsq(X, y, rcond=None)[0]
# PY:     e = y - X @ beta
# PY:     yhat = X @ beta
# PY:     sse = float(np.sum(e ** 2))
# PY:     sst = float(np.sum((y - y.mean()) ** 2))
# PY:     r2 = 1 - sse / sst
# PY:     idx = names if names is not None else list(range(len(beta)))
# PY:     return pd.Series(beta, index=idx), e, yhat, r2, sse

# PY: y = sub["lwage"].to_numpy(float)
# PY: X = np.column_stack([
# PY:     sub["education"].to_numpy(float),
# PY:     sub["experience"].to_numpy(float),
# PY:     sub["exp2"].to_numpy(float),
# PY:     np.ones(len(sub)),
# PY: ])
# PY: names = ["education", "experience", "exp2/100", "intercept"]
# PY: beta, e, yhat, r2, sse = ols(y, X, names)
# PY: print(beta.to_string())
# PY: print(f"R^2 = {r2:.6f}")
# PY: print(f"SSE = {sse:.6f}")
# PY: print("Book (3.49) approx: 0.144, 0.043, -0.095, 0.531")
...
# PY: Z = np.column_stack([
# PY:     sub["experience"].to_numpy(float),
# PY:     sub["exp2"].to_numpy(float),
# PY:     np.ones(len(sub)),
# PY: ])
# PY: edu = sub["education"].to_numpy(float)

# PY: b_y = np.linalg.lstsq(Z, y, rcond=None)[0]
# PY: b_e = np.linalg.lstsq(Z, edu, rcond=None)[0]
# PY: ry = y - Z @ b_y
# PY: re = edu - Z @ b_e

# PY: Xfwl = np.column_stack([re, np.ones(len(re))])
# PY: beta_fwl, e_fwl, _, r2_fwl, sse_fwl = ols(ry, Xfwl, ["education (FWL)", "intercept"])
# PY: print(beta_fwl.to_string())
# PY: print(f"FWL R^2 = {r2_fwl:.6f}")
# PY: print(f"FWL SSE = {sse_fwl:.6f}")
# PY: print("slope equals full OLS?", np.isclose(beta_fwl.iloc[0], beta["education"]))
# PY: print("SSE equals full OLS?", np.isclose(sse_fwl, sse))
# PY: print("R^2 equal? (expect False)", np.isclose(r2_fwl, r2))
...
# PY: X1 = sub["education"].to_numpy(float)
# PY: X2 = sub["experience"].to_numpy(float)

# PY: checks = {
# PY:     "(a) sum e": np.sum(e),
# PY:     "(b) sum X1*e": np.sum(X1 * e),
# PY:     "(c) sum X2*e": np.sum(X2 * e),
# PY:     "(d) sum X1^2*e": np.sum(X1**2 * e),
# PY:     "(e) sum X2^2*e": np.sum(X2**2 * e),
# PY:     "(f) sum Yhat*e": np.sum(yhat * e),
# PY:     "(g) sum e^2": np.sum(e**2),
# PY: }
# PY: for k, v in checks.items():
# PY:     print(f"{k:20s} = {v: .6e}")

# PY: print()
# PY: print("Theory notes:")
# PY: print("- (a)(b)(c)(f) ~ 0: intercept, edu, exp in X; Yhat in col(X)")
# PY: print("- (e) ~ 0: exp^2/100 is in the regression")
# PY: print("- (d) != 0: education^2 not in the regression")
# PY: print("- (g) = SSE")
...
# PY: mask_w = (df["race"] == 1) & (df["female"] == 0) & (df["hisp"] == 1)
# PY: s2 = df.loc[mask_w].copy()
# PY: print("n white male Hispanic =", len(s2))

# PY: s2["married"] = s2["marital"].isin([1, 2, 3]).astype(float)
# PY: s2["wid_div"] = s2["marital"].isin([4, 5]).astype(float)
# PY: s2["separated"] = (s2["marital"] == 6).astype(float)
# PY: s2["NE"] = (s2["region"] == 1).astype(float)
# PY: s2["South"] = (s2["region"] == 3).astype(float)
# PY: s2["West"] = (s2["region"] == 4).astype(float)

# PY: y2 = s2["lwage"].to_numpy(float)
# PY: X2m = np.column_stack([
# PY:     s2["education"], s2["experience"], s2["exp2"],
# PY:     s2["NE"], s2["South"], s2["West"],
# PY:     s2["married"], s2["wid_div"], s2["separated"],
# PY:     np.ones(len(s2)),
# PY: ])
# PY: names2 = [
# PY:     "education", "experience", "exp2/100",
# PY:     "Northeast", "South", "West",
# PY:     "married", "widowed/divorced", "separated",
# PY:     "intercept",
# PY: ]
# PY: beta2, e2, yhat2, r2_2, sse2 = ols(y2, X2m, names2)
# PY: print(beta2.to_string())
# PY: print(f"R^2 = {r2_2:.6f}, SSE = {sse2:.6f}")
...
# PY:     res = sm.OLS(y2, X2m).fit()
# PY:     cmp = pd.DataFrame({
# PY:         "numpy_lstsq": beta2.values,
# PY:         "statsmodels": res.params,
# PY:         "abs_diff": np.abs(beta2.values - res.params),
# PY:     }, index=names2)
# PY:     display(cmp) if "display" in dir() else print(cmp)
# PY:     print("max abs diff =", float(cmp["abs_diff"].max()))
# PY: except Exception as ex:
# PY:     print("statsmodels unavailable or error:", ex)
# PY:     print(beta2)
...
# PY: n, k = 80, 4
# PY: X = np.column_stack([rng.standard_normal((n, k-1)), np.ones(n)])
# PY: beta = np.array([1.0, -0.5, 2.0, 0.3])
# PY: Y = X @ beta + rng.standard_normal(n) * 2.0
# PY: XtXinv = np.linalg.inv(X.T @ X)
# PY: P = X @ XtXinv @ X.T
# PY: M = np.eye(n) - P
# PY: beta_hat = np.linalg.solve(X.T @ X, X.T @ Y)
# PY: e = Y - X @ beta_hat
# PY: h = np.diag(P)
# PY: print("[基本] X'e≈0:", np.allclose(X.T @ e, 0),
# PY:       " P²=P:", np.allclose(P @ P, P),
# PY:       " tr(P)=k:", np.isclose(np.trace(P), k),
# PY:       " tr(M)=n-k:", np.isclose(np.trace(M), n-k))

# Ex 3.15: 含截距时 R² = Corr(Y, Ŷ)²
# PY: yhat = X @ beta_hat
# PY: r2 = 1 - (e @ e) / ((Y - Y.mean()) @ Y)
# PY: print(f"[3.15] R²={r2:.4f}  Corr²={np.corrcoef(Y, yhat)[0,1]**2:.4f}  (应相等)")

# Ex 3.17: 留一误差 σ̃² ≥ 样本内残差 σ̂²（高杠杆点被放大）
# PY: sig2hat = (e @ e) / n
# PY: sig2tilde = np.mean((e / (1 - h))**2)
# PY: print(f"[3.17] σ̂²={sig2hat:.4f} ≤ σ̃²={sig2tilde:.4f}: {sig2tilde >= sig2hat}")

# Ex 3.18 & 3.20: 删一估计（暴力删行 vs 公式）
# PY: i = 3
# PY: Xmi, Ymi = np.delete(X, i, 0), np.delete(Y, i)
# PY: beta_loo = np.linalg.solve(Xmi.T @ Xmi, Xmi.T @ Ymi)
# PY: e_loo_pred = e[i] / (1 - h[i])                                  # Theorem 3.7 的预测误差
# PY: sse_loo = ((Ymi - Xmi @ beta_loo)**2).sum()
# PY: sig2loo_formula = n / (n-1) * sig2hat - e[i]**2 / ((n-1) * (1 - h[i]))   # Ex 3.20 公式
# PY: print(f"[3.18] β̂(-i) 与 β̂ 的差={np.abs(beta_loo - beta_hat).max():.2e} (ê_i={e[i]:.3f}; 差=0 iff ê_i=0)")
# PY: print(f"[3.20] σ²(-i) 暴力={sse_loo/(n-1):.5f}  公式={sig2loo_formula:.5f}  (应一致)")

# Ex 3.19: 截距模型 LOO = n/(n-1)(Y - Ȳ)
# PY: Yc = rng.standard_normal(n) * 2 + 5
# PY: eh = Yc - Yc.mean()
# PY: print(f"[3.19] 截距LOO匹配: {np.allclose(eh / (1 - 1/n), n/(n-1) * (Yc - Yc.mean()))}")

# Ex 3.10: 回归元正交分块时 P = P1 + P2
# PY: X1 = rng.standard_normal((n, 2))
# PY: Z = rng.standard_normal((n, 2))
# PY: X2 = Z - X1 @ np.linalg.solve(X1.T @ X1, X1.T @ Z)              # 残差化使 X1'X2 = 0
# PY: Xo = np.column_stack([X1, X2])
# PY: Po = Xo @ np.linalg.inv(Xo.T @ Xo) @ Xo.T
# PY: P1 = X1 @ np.linalg.solve(X1.T @ X1, X1.T)
# PY: P2 = X2 @ np.linalg.solve(X2.T @ X2, X2.T)
# PY: print(f"[3.10] X1'X2≈0: {np.allclose(X1.T @ X2, 0)}   P=P1+P2: {np.allclose(Po, P1 + P2)}")
