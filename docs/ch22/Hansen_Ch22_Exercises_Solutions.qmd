---
title: "Bruce Hansen《Econometrics》第 22 章习题解答"
subtitle: "Chapter 22 M-Estimators"
...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch22.R` 优先；否则为带注释的翻译草稿）。
...
# PY: rng = np.random.default_rng(22)

# 设定: Y = beta*X + e, e ~ N(0, sigma^2), X ~ N(0,1)
# 四次方损失 g(u)=u^4/4: minimize sum (Y-X*theta)^4 / 4
# 一阶条件: sum (Y_i - X_i*theta)^3 * X_i = 0 (凸问题, 唯一解)

# PY: n = 500; reps = 2000; beta_true = 1.0; sigma = 1.0
# PY: ols_est, quartic_est = [], []
# PY: for r in range(reps):
# PY:     X = rng.standard_normal(n)
# PY:     e = rng.standard_normal(n) * sigma
# PY:     Y = beta_true * X + e
    # OLS: minimize sum (Y-X*theta)^2
# PY:     b_ols = np.sum(X * Y) / np.sum(X ** 2)
# PY:     ols_est.append(b_ols)
    # 四次方损失: minimize sum (Y-X*theta)^4 / 4 (凸, 用有界优化)
# PY:     res = minimize_scalar(lambda th: np.sum((Y - X * th) ** 4),
# PY:                           bounds=(b_ols - 2, b_ols + 2), method='bounded')
# PY:     quartic_est.append(res.x)

# PY: ols_est = np.array(ols_est)
# PY: quartic_est = np.array(quartic_est)
# PY: ratio = np.var(quartic_est) / np.var(ols_est)
# PY: print(f"[Ex 22.3] 正态误差下四次方损失 vs OLS:")
# PY: print(f"  OLS 方差     = {np.var(ols_est):.6f}")
# PY: print(f"  四次方方差   = {np.var(quartic_est):.6f}")
# PY: print(f"  方差比       = {ratio:.4f} (理论 5/3 = {5/3:.4f})")
# PY: print(f"  ⇒ 四次方损失比 OLS 效率低约 {ratio:.2f} 倍 (OLS 是正态 MLE, 最优)")
