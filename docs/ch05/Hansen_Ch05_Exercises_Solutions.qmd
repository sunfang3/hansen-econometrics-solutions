---
title: "Bruce Hansen《Econometrics》第 5 章习题解答"
subtitle: "Chapter 5 Normal Regression"
...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch05.R` 优先；否则为带注释的翻译草稿）。
...
# PY: r = 5
# PY: Z = rng.normal(size=(200000, r))
# PY: Q = np.sum(Z**2, axis=1)
# PY: print("E[Q] sample", Q.mean(), "theory", r)
# PY: print("Var[Q] sample", Q.var(ddof=1), "theory", 2*r)
...
### 理论结论的蒙特卡洛验证（正态回归，无需外部数据）

以下单元格核对 ch05 的精确分布：$\chi^2$ 矩、拟合值方差 $\sigma^2 h_{ii}$、$\hat\beta$ 与 $s^2$ 独立、$\mathrm{var}(s^2)$ 与 CRLB。可独立运行。
...
# import numpy as np
# PY: rng = np.random.default_rng(5)

# 正态回归设定（误差 i.i.d. N(0,σ²)），条件于 X 做大量重复
# PY: n, k, reps = 60, 4, 60000
# PY: Xc = rng.standard_normal((n, k-1))
# PY: X = np.c_[Xc, np.ones(n)]
# PY: beta = np.array([1.0, -0.5, 0.7, 2.0])
# PY: sigma2 = 4.0
# PY: XtXinv = np.linalg.inv(X.T @ X)
# PY: h = np.diag(X @ XtXinv @ X.T)

# PY: Bh = np.zeros((reps, k)); Yhat3 = np.zeros(reps); s2 = np.zeros(reps); chic = np.zeros(reps)
# PY: for r in range(reps):
# PY:     e = rng.standard_normal(n) * np.sqrt(sigma2)
# PY:     Y = X @ beta + e
# PY:     bh = XtXinv @ (X.T @ Y)
# PY:     eh = Y - X @ bh
# PY:     Bh[r] = bh
# PY:     Yhat3[r] = X[3] @ bh
# PY:     s2[r] = (eh @ eh) / (n - k)
# PY:     chic[r] = (n - k) * s2[r] / sigma2

# Ex 5.1: χ²_r 的均值=r、方差=2r
# PY: Q = np.sum(rng.standard_normal((200000, 5))**2, axis=1)
# PY: print(f"[5.1]  χ²_5: E={Q.mean():.3f}(应5)  Var={Q.var():.3f}(应10)")

# Ex 5.5: 拟合值方差 = σ² h_ii
# PY: print(f"[5.5]  var(Ŷ_i)={Yhat3.var():.5f} ≈ σ²·h_ii={sigma2*h[3]:.5f}")

# Ex 5.6/5.7: 正态下 β̂ 与 s² 不相关 ⇒ 独立
# PY: print(f"[5.6/7] cov(β̂,s²)≈0:", np.round(np.cov(Bh.T, s2)[:k, -1], 4))

# Ex 5.12: (n-k)s²/σ² ~ χ²_{n-k}；var(s²)=2σ⁴/(n-k) > CRLB=2σ⁴/n
# PY: print(f"[5.12] (n-k)s²/σ²: 均值={chic.mean():.2f}(={n-k})  方差={chic.var():.2f}(={2*(n-k)})")
# PY: print(f"       var(s²)={s2.var():.5f}=2σ⁴/(n-k)={2*sigma2**2/(n-k):.5f} > CRLB 2σ⁴/n={2*sigma2**2/n:.5f}")
