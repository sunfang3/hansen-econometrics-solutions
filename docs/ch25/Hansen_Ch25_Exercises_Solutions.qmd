...
- **代码：** Python notebook 已融合为 **R**（`scripts/r_companions/ch25.R` 优先；否则为带注释的翻译草稿）。
...
手写/精译 R 脚本：`scripts/r_companions/ch25.R`。

```{r}
#| label: companion-main
source(file.path(root, "scripts", "r_companions", "ch25.R"))
```
